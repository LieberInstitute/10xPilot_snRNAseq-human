...
dim(sce.amy)
# [1] 33538  6632
length(unique(colnames(sce.amy)))
# [1] 6629

col_tab <- table(colnames(sce.amy))
col_tab[col_tab > 1]
# CAGTTCCTCTATTTCG-1 CGATCGGTCGGCATAT-1 GAAGCGATCGGTAGAG-1
# 2                  2                  2

length(unique(rownames(sce.amy)))
# [1] 33538

sce_amy_small <- sce.amy
assays(sce_amy_small) <- assays(sce_amy_small)["logcounts"]
rowData(sce_amy_small)
# sce_amy_small$Sample <- factor(gsub(".*premRNA/|/outs/.*", "", sce_amy_small$Sample))
sce_amy_small$sample <- factor(sce_amy_small$sample)
sce_amy_small$region <- factor(sce_amy_small$region)
sce_amy_small$donor <- factor(sce_amy_small$donor)
sce_amy_small$processDate <- factor(sce_amy_small$processDate)
sce_amy_small$protocol <- factor(sce_amy_small$protocol)
colData(sce_amy_small) <- colData(sce_amy_small)[, !colnames(colData(sce_amy_small)) %in% c("Sample", "prelimCluster", "collapsedCluster", "lessCollapsed", "cellType", "cellType.split", "prelimCluster.split", "cellType.moreSplit", "cellType.final")]
sce_amy_small$cell_type <- factor(sce.amy$cellType.split)
colData(sce_amy_small)

## Make the rows more browsable
colnames(sce_amy_small) <- paste0(sce_amy_small$sample, '_', sce_amy_small$Barcode)
sce_amy_small$Barcode <- make.names(sce_amy_small$Barcode, unique = TRUE)
metadata(sce_amy_small) <- list()
sce_amy_small

## It's all "Gene Expression", so we can remove it
rowData(sce_amy_small)$Type <- NULL
